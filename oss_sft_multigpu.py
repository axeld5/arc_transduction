...
def load_all_training_data(
    ...
    # Convert to GPT-OSS conversation format with reasoning
    conversations = []
    for ex in all_data:
        # Build developer message content with system prompt if provided
        ...
        # Create conversation with GPT-OSS format
        conversation = [
            {"role": "developer", "content": developer_content},
            {"role": "user", "content": ex['problem']},
            # Assistant response with thinking and final answer
...
def run_sft(
    ...
    """
    print(f"\n{'='*80}")
    print("GPT-OSS TRANSDUCTION SFT TRAINING (MULTI-GPU)")
    print(f"{'='*80}")
    ...
    # Setup model - GPT-OSS requires special handling
    print(f"\nLoading model: {base_model}")
    print("Note: GPT-OSS will use float32 precision")
    
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=base_model,
        max_seq_length=max_seq_length,
        dtype=None,  # Auto-detect, will use float32 for gpt-oss
    ...
    # Run evaluation if requested (simplified for GPT-OSS)
    eval_results = None
    if eval_after_training and local_rank == 0:
        print("\n[SFT] Note: Evaluation for GPT-OSS models requires special setup.")
        print("[SFT] Please run evaluation separately using the saved model.")
        # TODO: Implement GPT-OSS specific evaluation if needed
...
if __name__ == "__main__":
    sft_model_save_path, sft_merged_save_path, eval_results = run_sft(
        ...
        eval_after_training=False,  # Set to False for now
    )
